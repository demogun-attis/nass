...
def get_wu_history(config):
    # API_URL = 'http://api.wunderground.com/api/{key}/history_{day}/q/{state}/{town}.json'
    API_URL = 'https://api.openweathermap.org/data/2.5/onecall/timemachine?lat={lat}&lon={lon}&dt={epoch}&appid={api_key}'
    return requests.get(API_URL.format(api_key=config['api_key'],
                                       epoch=get_24h_before_epoch(),
                                       lat=config['lat'],
                                       lon=config['lon']))
...
def get_precip_in_window(config, time_win_hr=24):
    ...
    try:
        r_yesterday = get_wu_history(config)
        t_yesterday, vals_yesterday = get_rainfall(r_yesterday)
    except Exception as ex: 
        return None
    
    #try:
    #    r_today = get_wu_history(config)
...
def get_24h_before_epoch():
  from datetime import datetime, time, timedelta
  from calendar import timegm
  
  midnight = datetime.now() 
  yesterday_midnight = midnight - timedelta(days=1)
  epoch_time = (yesterday_midnight - datetime(1970, 1, 1)).total_seconds()
  return int(epoch_time)

def report():
    print "Reporting to SQL"
...
def run_sprinkler(config):
  ...
  pin = ['2', '3', '4', '17', '27', '22', '9']
  # time to sleep between operations in the main loop
  fieldsfilename = 'fields.ini'
  fields = ConfigParser.RawConfigParser()
  fields.read(fieldsfilename)
  ...
  with open(config['log_file'],'a') as log_file:
    try:
      ...
      log_file.write('%s: Starting %s\n' % (now(), pump))
      print "%s: Starting %s\n" % (now(), pump)
      GPIO.output((10, led), GPIO.HIGH)

      for i in pin:
        sprinkler_name = fields.get(i, 'name')
        GPIO.setup((int(i), led), GPIO.OUT)
        log_file.write('%s: Starting sprinkler: %s\n' % (now(), sprinkler_name))
        print "%s: Starting sprinkler: %s\n" % (now(), sprinkler_name)
        GPIO.output((int(i), led), GPIO.HIGH)
        sleep(runtime * 60)
        sleep(20)
        log_file.write('%s: Stopping sprinkler: %s\n' % (now(), sprinkler_name))
        print "%s: Stopping sprinkler: %s\n" % (now(), sprinkler_name)
        GPIO.output((int(i), led), GPIO.LOW)
      
      log_file.write('%s: Stopping %s\n' % (now(), pump))
      print "%s: Stopping %s\n" % (now(), pump)
